...
def format_labels(path_points_poly, output_dir='outputs'):
    ...
    pts_gdf.drop(columns=['Shape_Leng', 'Shape_Area'], inplace=True, errors='ignore')
    pts_gdf['CATEGORY'] = [str(int(code)) + color if color else 'undetermined' for code, color in zip(pts_gdf.Code_type_, pts_gdf.Couleur)] 
